...
def compute_C(E=1., nu=0.3, sy=0.01, abqlauncher='/opt/Abaqus/6.9/Commands/abaqus', workdir='workdir', name='indentation_axi_fancier', frames=50):
    ...
    * Load prefactor C (float)
    ...
    p = subprocess.Popen('{0} job={1} input={1}.inp interactive ask_delete=OFF'.format(
        abqlauncher, name), cwd=workdir, shell=True, stdout=subprocess.PIPE)
    trash = p.communicate()

    # Now we test run the post processing script
    print ('Post processing the simulation in Abaqus/Python')
    p = subprocess.Popen(
        [abqlauncher, 'viewer', 'noGUI=fancier_example_abq.py'], cwd='.', stdout=subprocess.PIPE)
    trash = p.communicate()
    ...
    if data['completed']:
        ...
        trash, force_l = force_hist[0, 1].plotable()
        # Getting backdisplacement during loading
        trash, disp_l = disp_hist[0, 1].plotable()
