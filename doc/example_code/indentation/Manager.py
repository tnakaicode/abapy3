...
indenter = DeformableCone2D(half_angle=half_angle,
                            Na=Na, Nb=Nb, Ns=Ns, Nf=Nf, rigid=rigid_indenter)
E = 1.                                                    # Young's modulus
...
if data['completed']:
    ...
    C = (F[1] / h[1]**2).average()
    F_fit = C * h ** 2
    plt.plot(h.plotable()[1], F.plotable()[1], 'b-',
             label='Simulated curve', linewidth=1.)
    plt.plot(h[0, 1].plotable()[1], F_fit[0, 1].plotable()[1],
             'r-', label='fitted loading curve', linewidth=1.)
    ...
    # Gradiant plot, Nlevels specifies the number of levels.
    grad = plt.tricontourf(x, y, tri, stress.data, levels)
