...
# Model Parameters
# Indenter half angle, 65.27 leads to a modified Berkovich geometry, see help(DeformableCone3D for further information)
half_angle = 65.27
# with 4, 4, 4, 2, 1., simulation is very fast, the mesh is coarse (even crappy) but the result is surprisingly good!
Na, Nb, Ns, Nf, l = 8, 8, 8, 2, 1.
sweep_angle, N = 60., 8
# E is the Young's modulus and nu is the Poisson's ratio.
E, nu = 1., 0.3
...
# Building model
indenter = DeformableCone3D(half_angle=half_angle, rigid=True, Na=Na, Nb=Nb,
                            Ns=Ns, Nf=Nf, N=N, sweep_angle=sweep_angle, l=l, pyramid=pyramid)  # Indenter
sample_mesh = IndentationMesh(Na=Na, Nb=Nb, Ns=Ns, Nf=Nf, l=l).sweep(
    sweep_angle=sweep_angle, N=N)   # Sample Mesh
# sample_mat = Elastic(labels = 'SAMPLE_MAT', E = E, nu=  nu)    # Sample material
sample_mat = VonMises(labels='SAMPLE_MAT', E=E, nu=nu,
                      sy=E * ey)    # Sample material
indenter_mat = Elastic(labels='INDENTER_MAT')    # Indenter material
