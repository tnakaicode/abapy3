...
dump(data, file_name+'.pckl')"""
    ...
    def Load_prefactor(self, update=False):
        ...
        '''
        load_prefactor = (
            self.force_hist[1] / self.disp_hist[1]**2).average(method='simps')
        if update:
    ...
    def Plastic_work_ratio(self, update=False):
        ...
        '''
        plastic_work_ratio = (
            self.plastic_work_hist[1] / self.total_work_hist[1]).average()
        if update:
    ...
    def Contact_area(self, update=False):
        ...
        # we only look at the loading 1 here
        contact_step = self.contact_data[1]
        ca = np.array([contact_step[i].contact_area()
                       for i in range(len(contact_step))])
    ...
    def difficulty(self):
        ...
        if self.sample_mat_type == 'hollomon':
            args = self.sample_mat_args
            mat_diff = args['young_modulus'] / args['yield_stress']
        mesh_diff = self.max_disp / self.mesh_l * \
            (self.mesh_Na + self.mesh_Nb) / 2.
...
class Database_Manager:
    ...
    def add_simulation(self, simulation):
        ...
        * simulation: Simulation class instance
        '''
        if isinstance(simulation, self.cls) == False:
            raise Exception('simulation must be Simulation instance')
        try:
            self.session.add(simulation)
            self.session.commit()
        except:
            print(
                'simulation already exists or has not been declared corectly, nothing changed')
    ...
    def get_next(self):
        ...
        '''
        simus = self.session.query(self.cls).filter(
            self.cls.completed == False)
        if simus.all() != []:
            # finding max priority
            max_priority = self.session.query(self.cls).filter(
                self.cls.completed == False).order_by(desc(self.cls.priority)).first().priority
            ...
            completed_simus = self.session.query(
                self.cls).filter(self.cls.completed == True)
            for csim in completed_simus:
                if csim.difficulty() <= diff:
                    if csim.frames > simu.frames:
                        simu.frames = csim.frames
            completed_simus = [
                c_simu for c_simu in completed_simus if c_simu.difficulty <= diff]
            self.session.commit()
        else:
            simu = None
        return simu
    ...
    def run_all(self):
        ...
        '''
        while True:
            left_sims = self.session.query(self.cls).filter(
                self.cls.completed == False).count()
            if left_sims == 0:
