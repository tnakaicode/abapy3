...
plt.plot(D_u, F_u_fit, 'b-',
         label=r'Unloading fit: $F = F_m \left(\frac{d - d_f}{d_m-d_f}\right)^m$', linewidth=lw)
...
plt.plot(R_ind_l / d_m, Ind_l / d_m, '-r',
         label='Indenter$_{h=h_m}$', linewidth=lw)
...
plt.plot(R_ind_u / d_m, Ind_u / d_m, '-r',
         label='Indenter$_{h=h_f}$', linewidth=lw)
plt.xticks([0., r_c], ['', '$r_c$'], fontsize=fs)
...
plt.xlabel('$r$', fontsize=fs)
plt.ylabel('$z$', fontsize=fs)
plt.plot(R_ind_v / d_m, Ind_v / d_m, '-g', linewidth=lw,
         label='Virtual Indenter = Indenter$_{h=h_f}$ - Sample$_{h=h_f}$')
d_u = d_m - d_f
# Interpolation function of the radius of the virtual indenter
r_ind = interpolate.interp1d(Ind_v / d_m, R_ind_v / d_m)
r_c_me = r_ind(.5 * d_u)
