...
class Element(object):
    ...
    def node_set_to_surface(self, nodesetlabel, surfacelabel):
        nodelabels = set([k for k in self.nodes.keys()
                          if label in self.nodes[k].sets])
        for element in self.elements.values:
            for i in range(self.ns()):
                if self._space == 3:
                    surfconn = self.conn[self._faces_conn[i]]
                # TO be completed
...
def parseInp(path):
    ...
    for line in lines:
        ...
        if d["type"] == "data":
            ...
            if env == "node":
                label = int(words[0])
                coords = np.array([float(w)
                                   for w in words[1:]], dtype=np.float64)
                if currentset == None:
                    m.nodes[label] = Node(coords=coords)
                else:
                    m.nodes[label] = Node(
                        coords=coords, sets=set([currentset]))
            ...
            if env == "element":
                ...
                if currentset == None:
                    ...
                    m.elements[label] = globals()[eltype](
                        conn=conn, sets=set([currentset]))

            if env == "nset":
                [m.nodes[int(w)].sets.add(currentset)
                 for w in words if len(w) != 0]

            if env == "elset":
                [m.elements[int(w)].sets.add(currentset)
                 for w in words if len(w) != 0]
...
def writeMsh(mesh, path=None):
    elementMap = {"Tri3": 2, "Quad4": 3, "Tetra4": 4,
                  "Hexa8": 5, "Prism6": 6, "Pyra5": 7}
    pattern = """$MeshFormat
2.2 0 8
$EndMeshFormat
$Nodes
...
$EndElements"""
    ...
    for k in mesh.nodes.keys():
        node = mesh.nodes[k]
        coords = node.coords
        nodelist.append("{0} {1} {2} {3}".format(
