...
def ParamInfiniteMesh(Na=10, Nb=10, l=1., core_name='CAX4', add_shell=True, shell_name='CINAX4', dti='I', dtf='d'):
    ...
    # permutes nodes in connectivity to have a particular orientation. Flips CLOCKWISE.
    def conn_permutation(conn):
        from copy import copy
        conn2 = copy(conn)
        for i in range(len(conn)):
            conn2[i] = conn[i - 1]
        return conn2
    m1 = RegularQuadMesh_like(linspace(
        0., l, Na + 1), linspace(-l, 0., Na + 1), name=core_name, dtf=dtf, dti=dti)
    ...
    x2 = [l]
    for i in range(Nb):
        x2.append(x2[-1] * Na / (Na - 1))
    y2 = linspace(-l, 0, Na + 1)
    m2 = RegularQuadMesh_like(x_list=x2, y_list=y2,
                              name=core_name, dtf=dtf, dti=dti)
    m2.nodes.sets = {}
    ...
    y3 = [-l]
    for i in range(Nb):
        y3.append(y3[-1] * Na / (Na - 1))
    y3.sort()
    x3 = linspace(0., l, Na + 1)
    m3 = RegularQuadMesh_like(x_list=x3, y_list=y3,
                              name=core_name, dtf=dtf, dti=dti)
    m3.nodes.sets = {}
    ...
    if add_shell:
        m4 = RegularQuadMesh_like(
            x_list=[x2[-1], 2 * x2[-1]], y_list=y2, name=shell_name, dtf=dtf, dti=dti)
        m4.nodes.sets = {}
        m4.add_set('shell', m4.labels)
        m5 = RegularQuadMesh_like(
            x_list=x3, y_list=[2 * y3[0], y3[0]], name=shell_name, dtf=dtf, dti=dti)
        m5.nodes.sets = {}
...
def IndentationMesh(Na=8, Nb=8, Ns=4, Nf=2, l=1., name='CAX4', dtf='f', dti='I'):
    ...
    def PolarShell(radius=1., N=2, Ns=4, theta0=-90., theta1=0., k=None, name='CAX4'):
        ...
        mesh = RegularQuadMesh_like(
            x_list=x1, y_list=y1, name=name, dtf=dtf, dti=dti)
        mesh.sets = {}
        mesh.nodes.sets = {}
    ...
    m1 = RegularQuadMesh_like(
        linspace(0., l, Na + 1), linspace(-l, 0., Nb + 1), name=name, dtf=dtf, dti=dti)
    m1.sets = {}
    m1.nodes.sets = {}
    ...
    x2 = [l]
    for i in range(Nmin):
        x2.append(x2[-1] * Nmin / (Nmin - 1))
    y2 = linspace(-l, 0, Nb + 1)
    m2 = RegularQuadMesh_like(x_list=x2, y_list=y2,
                              name=name, dtf=dtf, dti=dti)
    y3 = [-l]
    for i in range(Nmin):
        y3.append(y3[-1] * Nmin / (Nmin - 1))
    y3.sort()
    x3 = linspace(0., l, Na + 1)
    m3 = RegularQuadMesh_like(x_list=x3, y_list=y3,
                              name=name, dtf=dtf, dti=dti)
    ...
    while Nta > Nf and Ntb > Nf:
        ...
        transa = PolarTransition(
            N=Nta, theta0=-90., theta1=-45., radius=radius, size=size)
        transb = PolarTransition(
            N=Ntb, theta0=-45., theta1=0., radius=radius, size=size)
    ...
    t = (pi / 4.) / min(Nta, Ntb)
    k = t / (1. - t / 2.)
    shella = PolarShell(radius=radius, N=Nta, Ns=Ns,
                        theta0=-90., theta1=-45., k=k)
    shellb = PolarShell(radius=radius, N=Ntb, Ns=Ns,
                        theta0=-45., theta1=0., k=k)
    ...
    for i in range(len(n1.x)):
        if n1.x[i] < crit_distance:
            n1.x[i] = 0.
        if n1.y[i] > -crit_distance:
            n1.y[i] = 0.
    # Managing sets
    m1.nodes.add_set_by_func('top_nodes', lambda x, y, z,
                             labels: logical_and(y == 0., x <= surface_radius))
    m1.nodes.add_set_by_func('surface_nodes', lambda x, y,
                             z, labels: logical_and(y == 0., x <= surface_radius))
    ...
    m1.add_set('all_elements', m1.labels)
    m1.nodes.add_set_by_func('tip_node', lambda x, y,
                             z, labels: logical_and(y == 0., x == 0.))
    m1.nodes.add_set_by_func('ref_node', lambda x, y, z,
                             labels: logical_and(y == min(m1.nodes.y), x == 0.))
    radius = max(m1.nodes.x)
    error = 0.001
    m1.nodes.add_set_by_func('bottom_nodes', lambda x, y, z, labels: (
        x**2 + y**2)**.5 > (1 - error) * radius)
...
class RigidCone2D:
    ...
    def set_summit_position(self, summit_position):
        ...
        :type summit_position: tuple or list containing two floats.
        '''
        if len(summit_position) != 2:
            raise Exception('summit_position length must be 2, got {0} instead.'.format(
                len(summit_position)))
        self.summit_position = (
            float(summit_position[0]), float(summit_position[1]))
    ...
    def set_half_angle(self, half_angle=70.3):
        ...
        if type(half_angle) not in [float, int]:
            raise Exception(
                'half angle must be float, got {0}'.format(type(half_angle)))
...
*RIGID BODY, REF NODE = REFERENCE, ANALYTICAL SURFACE = IND_SURF'''
    ...
    def apply_displacement(self, disp):
        ...
        if isinstance(disp, VectorFieldOutput) == False:
            raise Exception(
                'disp must be VectorFieldOutput instance, got {0} instead'.format(type(disp)))
...
class DeformableCone2D:
    ...
    def set_mat_label(self, mat_label):
        ...
        if type(mat_label) is not str:
            raise Exception(
                'mat_label must be string, got {0}'.format(type(mat_label)))
    ...
    def set_rigid(self, rigid):
        ...
        if type(rigid) is not bool:
            raise Exception(
                'rigid must be bool type, got {0} instead'.format(type(rigid)))
        self.rigid = rigid

    def make_mesh(self):
        from numpy import tan, radians, pi, arcsin, sin, cos
        mesh = IndentationMesh(Na=self.Na, Nb=self.Nb,
                               Ns=self.Ns, Nf=self.Nf, l=self.l, name='CAX4')
    ...
    '''
        ...
        mesh.nodes.translate(
            x=self.summit_position[0], y=self.summit_position[1])
        self.mesh = mesh
...
class DeformableCone3D:
    ...
    def set_sweep_angle(self, sweep_angle):
        ...
        if type(sweep_angle) != float:
            raise Exception(
                'sweep_angle must be float, got {0}'.format(type(sweep_angle)))
    ...
    def set_pyramid(self, pyramid):
        ...
        :type pyramid: bool
        '''
        if type(pyramid) is not bool:
            raise Exception(
                'pyramid must be bool type, got {0} instead'.format(type(pyramid)))
    ...
    def make_mesh(self):
        ...
        mesh.add_surface('surface_faces', [('surface_elements', 1)])
        mesh = mesh.sweep(
            N=self.N, sweep_angle=self.sweep_angle, extrude=self.pyramid)
        # Removing some sets intersections
        mesh.nodes.sets['all_front_nodes'] = copy.copy(
            mesh.nodes.sets['front_nodes'])
        mesh.nodes.sets['all_back_nodes'] = copy.copy(
            mesh.nodes.sets['back_nodes'])
...
class Step(object):
    ...
    def __init__(self, name, disp=1., nlgeom=True, nframes=100, fieldOutputFreq=999999, boundaries_3D=False, full_3D=False, rigid_indenter_3D=True, nodeFieldOutput=['COORD', 'U'], elemFieldOutput=['LE', 'EE', 'PE', 'PEEQ', 'S'], mode='bulk'):
        ...
        if mode not in modes:
            raise Exception('mode must be in {0}'.format(modes))
        self.mode = mode
    ...
    def set_name(self, name):
        ...
        if type(name) is not str:
            raise Exception('name must be str, got {0}'.format(type(name)))
        self.name = name
    ...
    def set_displacement(self, disp):
        ...
        :type disp: float > 0.
        '''
        if type(disp) not in [float, int]:
            raise Exception('disp must be float, got {0}'.format(type(disp)))
    ...
    def set_nframes(self, nframes):
        ...
        :type nframes: int
        '''
        if type(nframes) not in [int]:
            raise Exception(
                'disp must be int > 0, got {0}'.format(type(nframes)))
    ...
    def set_nlgeom(self, nlgeom):
        ...
        :type nlgeom: boolean
        '''
        if type(nlgeom) is not bool:
            raise Exception(
                'nlgeom must be boolean, got {0}'.format(type(nlgeom)))
        self.nlgeom = nlgeom
    ...
    def set_fieldOutputFreq(self, freq):
        ...
        :type freq: int
        '''
        if type(freq) not in [int]:
            raise Exception('freq must be int, got {0}'.format(type(freq)))
    ...
    def set_boundaries_3D(self, boundaries_3D):
      ...
      :type boundaries_3D: boolean 
        '''
        if type(boundaries_3D) != bool:
            raise Exception(
                'boundaries_3D must be bool, got {0}'.format(type(boundaries_3D)))
    ...
    def set_full_3D(self, full_3D):
      ...
      :type full_3D: boolean 
        '''
        if type(full_3D) != bool:
            raise Exception(
                'full_3D must be bool, got {0}'.format(type(full_3D)))
    ...
    def set_rigid_indenter_3D(self, rigid_indenter_3D):
      ...
      :type rigid_indenter_3D: boolean
        '''
        if type(rigid_indenter_3D) != bool:
            raise Exception('rigid_indenter_3D must be bool, got {0}'.format(
                type(rigid_indenter_3D)))
        ...
        if type(nodeOutput) is str:
            ...
            for no in nodeOutput:
                if type(no) is not str:
                    raise Exception(
                        'node outputs must be strings, got {0}'.format(type(no)))
            self.nodeFieldOutput = nodeOutput
    ...
    def set_elemFieldOutput(self, elemOutput):
        ...
        if type(elemOutput) is str:
            ...
            for eo in elemOutput:
                if type(eo) is not str:
                    raise Exception(
                        'element outputs must be strings, got {0}'.format(type(eo)))
            self.elemFieldOutput = elemOutput
...
'''
        ...
        for fo in self.nodeFieldOutput:
            nfos += '{0}, '.format(fo)
        out = stepPattern.format(self.name.upper(
        ), nlgeom, 1. / self.nframes, -self.disp, self.fieldOutputFreq, nfos, efos, bc_3D)
        return out
...
0., 0., 0., 0., 1.,0.'''
        # Removing some sets intersections
        sample_mesh.nodes.sets['all_front_nodes'] = copy.copy(
            sample_mesh.nodes.sets['front_nodes'])
        sample_mesh.nodes.sets['all_back_nodes'] = copy.copy(
            sample_mesh.nodes.sets['back_nodes'])
        ...
        csys = ''
    if steps == None:
        steps = [Step(name='LOADING', disp=1.),
                 Step(name='UNLOADING', disp=0.)]
    if isinstance(steps, Step):
        steps = [steps]
    sample_mat.labels = ['SAMPLE_MAT']
    indenter_mat.labels = ['INDENTER_MAT']
    out = introPattern.format(sample_mesh.dump2inp(), indenter.dump2inp(
    ), friction, sample_mat.dump2inp(), indenter_mat.dump2inp(), csys, heading)
    for step in steps:
        out += step.dump2inp()
...
class Manager:
    ...
    def set_workdir(self, workdir):
        ...
        if type(workdir) is not str:
            raise Exception(
                'workdir must be string, got {0} instead.'.format(type(workdir)))
    ...
    def set_abqlauncher(self, abqlauncher):
        ...
        '''
        if type(abqlauncher) is not str:
            raise Exception(
                'workdir must be string, got {0} instead.'.format(type(abqlauncher)))
    ...
    def set_samplemesh(self, samplemesh):
        ...
        if isinstance(samplemesh, Mesh) == False:
            raise Exception(
                'samplemesh must be Mesh instance, got {0} instead.'.format(type(samplemesh)))
    ...
    def set_steps(self, steps):
        ...
        '''
        for step in steps:
            if isinstance(step, Step) == False:
                raise Exception(
                    'step must be Step instance, got {0} instead.'.format(type(step)))
        self.steps = steps
    ...
    def set_simname(self, simname):
        ...
        :type simname: string
        '''
        if type(simname) is not str:
            raise Exception(
                'simname must be string, got {0} instead.'.format(type(simname)))
    ...
    def set_files2delete(self, files2delete):
        ...
        for f in files2delete:
            if type(f) is not str:
                raise Exception(
                    'file must be string, got {0} instead'.format(type(f)))
    ...
    def make_inp(self):
        ...
        '''
        out = MakeInp(sample_mesh=self.samplemesh, indenter=self.indenter, sample_mat=self.samplemat,
                      indenter_mat=self.indentermat, friction=self.friction, steps=self.steps, is_3D=self.is_3D)
    ...
    def run_sim(self):
        ...
        command = '{0} job={1} input={1}.inp interactive'.format(
            self.abqlauncher, self.simname)
        print(command)
        p = subprocess.Popen(command, cwd=self.workdir,
        ...
        if isinstance(self.samplemat, SiDoLo):
            ...
            command = '{0} job={1} input={1}.inp user={2} interactive'.format(
                self.abqlauncher, self.simname, self.samplemat.umat[0])
            print(command)
            p = subprocess.Popen(command, cwd=self.workdir,
                                 shell=True, stdout=subprocess.PIPE)
        trash = p.communicate()
        t1 = time.time()
        self.duration = t1 - t0
        print('< Ran {0} in Abaqus: duration {1:.2f}s>'.format(
            self.simname, t1 - t0))
    ...
    def set_abqpostproc(self, abqpostproc):
        ...
        '''
        if type(abqpostproc) is not str:
            raise Exception('abqpostproc must be string')
    ...
    def run_abqpostproc(self):
        ...
        p = subprocess.Popen([self.abqlauncher, 'viewer', 'noGUI={0}'.format(
            self.abqpostproc)], cwd=self.workdir, stdout=subprocess.PIPE)
        trash = p.communicate()
        t1 = time.time()
        print('< Post Processed {0} in Abaqus: duration {1:.2f}s>'.format(
            self.simname, t1 - t0))
    ...
    def run_pypostproc(self):
        ...
        t1 = time.time()
        print('< Post Processed {0} in Python: duration {1:.2f}s>'.format(
            self.simname, t1 - t0))
        return data
  ...
  .. plot:: example_code/indentation/ContactData.py
    ...
    def add_data(self, coor1, coor2=0., altitude=0., pressure=0.):
        ...
        if hasattr(coor1, '__contains__'):
            ...
            if hasattr(altitude, '__contains__'):
                ...
                altitude = array(
                    self.dtf, [altitude for i in range(len(coor1))])
            if hasattr(pressure, '__contains__'):
                pressure = array(self.dtf, pressure)
            else:
                pressure = array(
                    self.dtf, [pressure for i in range(len(coor1))])
            self.coor1 += coor1
            self.coor2 += coor2
    ...
    def interpolate(self, coor1, coor2, axi_repeat=100, delaunay_disp=None, method='linear'):
      ...
      .. plot:: example_code/indentation/ContactData-interpolate.py
        ...
        import numpy as np
        points, alt, press, conn = self.get_3D_data(
            axi_repeat=axi_repeat, delaunay_disp=delaunay_disp)
    ...
    def contact_contour(self, delaunay_disp=None):
        ...
        x, y = points[:, 0], points[:, 1]
        fig = plt.figure()
        c = plt.tricontour(x, y, conn, norm_press, levels=[
                           0.0001, 0.5, 0.9999]).collections
...
def Get_ContactData(odb, instance, node_set):
    ...
    * As a consequence, coor1 will be x, coor2 will be z and the altitude is y.
    ...
    if odb.rootAssembly.instances[instance].embeddedSpace == THREE_D:
        is_3D = True
    ...
    for label in topNodes:
        available_hist = odb.steps[odb.steps.keys(
        )[0]].historyRegions[hist_region_key_pattern + str(label)].historyOutputs.keys()
        cpress_available = False
        for key in available_hist:
            if 'CPRESS' in key:
                cpress_available = True
                hist_target_key = key
        if cpress_available == False:
            topNodes.remove(label)
    # Getting data
    coor1 = gho(odb, 'COOR1')
    if is_3D:
        coor2 = gho(odb, 'COOR3')  # Only in 3D
    ...
    for st in range(n_steps):
        ...
        for fr in range(len(Coor1[0][st])):  # fr in the indice of the frame
            ...
            cd.add_data(coor1=coor1_temp, coor2=coor2_temp,
                        altitude=altitude_temp, pressure=pressure_temp)
...
class Hertz(object):
    ...
    def __init__(self, F=1., a=None, h=None, R=1., E=1., nu=0.3):
        ...
        if (a, F, h) == (None, None, None):
            raise ValueError(
                'a, F or h are not define, one at least must be defined')
...
class Hanson(object):
    ...
    def __init__(self, F=None, a=None, h=None, half_angle=70.29, E=1., nu=0.3):
        ...
        if (a, F, h) == (None, None, None):
            raise ValueError(
                'a, F or h are not defined, at least one must be defined')
        if a != None:
            self.a = a
        if F != None:
            self.F = F
