...
class Nodes(object):
    ...
    def __init__(self, labels=[], x=[], y=[], z=[], sets={}, dtf='f', dti='I'):
        ...
        if len(set([len(labels), len(x), len(y), len(z)])) != 1:
            raise ValueError(
                'labels, x, y and z args should have the same length')
    ...
    def add_node(self, label=None, x=0., y=0., z=0., toset=None):
        ...
        if len(labels) == 0:
            if label == None:
                label = 1
            if label < 1:
                raise Exception('node labels must be > 0.')
            labels.append(label)
            X.append(x)
            Y.append(y)
            Z.append(z)
            ...
            if label == None:
                ...
                if label not in labels:
                    ...
                    print(
                        'Info: node with label {0} already exists, nothing changed.'.format(label))
                    return
        if toset != None:
            ...
            for node in nodes:
                if node in self.labels:
                    if node not in self.sets[label]:
                        ...
                        print('Info: node {0} was already in set {1}, node was not added to set'.format(
                            node, label))
                else:
                    print(
                        'Info: node {0} does not exist, node was not added to set'.format(node))
    ...
    def add_set_by_func(self, name, func):
        ...
        if type(name) is not str:
            raise Exception(
                'set labels must be strings, got {0}.'.format(type(name)))
        if self.dtf == 'f':
            n_dtf = float32
        if self.dtf == 'd':
            n_dtf = float64
    ...
    def drop_set(self, label):
        ...
        if label in self.sets.keys():
            ...
            print(
                'Info: sets {0} does not exist and cannot be dropped.'.format(label))
    ...
    def eval_function(self, function):
        ...
        from numpy import float32, float64, uint32, uint64, float16
    ...
    def eval_tensorFunction(self, function):
        ...
        labels = self.labels
        n_labels = n_array(labels, dtype=n_dti)
        data11, data22, data33, data12, data13, data23 = function(
            x, y, z, n_labels)
    ...
    def apply_displacement(self, disp):
        ...
        if n_labels != u_labels:
            raise Exception(
                'Nodes labels and VectorFieldOutput labels must be identical.')
        n1, n2, n3 = n_array(self.x, dtype=n_dtf), n_array(
            self.y, dtype=n_dtf), n_array(self.z, dtype=n_dtf)
        u1, u2, u3 = n_array(disp.data1, dtype=n_dtf), n_array(
            disp.data2, dtype=n_dtf), n_array(disp.data3, dtype=n_dtf)
    ...
    def replace_node(self, old, new):
        ...
        x, y, z = self.x, self.y, self.z
        # Some verifications
        if old not in nlabels or new not in nlabels:
            raise Exception('Both node labels must exist.')
        # Let's proceed
    ...
    def apply_reflection(self, point=(0., 0., 0.), normal=(1., 0., 0.)):
        ...
        from array import array as a_array
        if len(point) != 3:
            raise Exception('point must be a list  of length 3.')
        if len(normal) != 3:
            raise Exception('normal must be a list  of length 3.')
        if self.dtf == 'f':
            a_dtf = float32
        if self.dtf == 'd':
            a_dtf = float64
        ...
        x, y, z = n_array(self.x, dtype=a_dtf), n_array(
            self.y, dtype=a_dtf), n_array(self.z, dtype=a_dtf)
        vx, vy, vz = x - xp, y - yp, z - zp
        vn = vx * xn + vy * yn + vz * zn
        vx, vy, vz = vx - 2 * vn * xn, vy - 2 * vn * yn, vz - 2 * vn * zn
        x, y, z = vx + xp, vy + yp, vz + zp
        out = copy(self)
        out.x, out.y, out.z = a_array(out.dtf, x.tolist()), a_array(
            out.dtf, y.tolist()), a_array(out.dtf, z.tolist())
        return out
...
class Mesh(object):
    ...
    def __init__(self, nodes=None, connectivity=[], space=[], labels=[], name=None, sets={}, surfaces={}):
        ...
        self.fields = {}
        l = len(labels)
        if len(connectivity) != len(labels):
            raise Exception(
                'connectivity must have the same length as labels.')
        if len(space) != len(labels):
            raise Exception('space must have the same length as labels.')
        if len(name) != len(labels):
            raise Exception(
                'if not None, name must have the same length as labels.')
        for i in range(len(labels)):
            self.add_element(
                label=labels[i], connectivity=connectivity[i], space=space[i], name=name[i])
        if type(sets) is not dict:
            raise Exception('sets type must be dict.')
        for k in sets.keys():
            self.add_set(k, sets[k])
        ...
        for l in labs:
            ...
            for sk in esets.keys():
                if l in esets[sk]:
                    toset.append(sk)
            E.add_element(
                label=l, connectivity=connectivity[i], space=space[i], name=name[i], toset=toset)
        nlabs = []
    ...
    def __str__(self):
        ...
        for i in range(len(labels)):
            out += '{0}\t{1}\t{2}\t{3}\n'.format(
                labels[i], c[i].tolist(), '{0}D'.format(s[i]), n[i])
        out += 'Sets:\nLabel\tElements\n'
        for k in self.sets.keys():
            out += '{0}\t'.format(k)
            for n in self.sets[k]:
    ...
    def add_element(self, connectivity, space, label=None, name=None, toset=None):
        ...
        if space not in [1, 2, 3]:
            raise Exception('space must be 1,2 or 3')
        if name == None:
            name = ''
        if type(name) is not str:
            raise Exception('name type must be str')
        # inputs processing
        if label in self.labels:
            print(
                'Info: element {0} already exists, nothing changed.'.format(label))
            return
        else:
            if len(self.labels) == 0:
    ...
    def drop_node(self, label):
        '''
        Drops a node from abapy.mesh.nodes instance. This method differs from to the nodes.drop_node element because it removes the node but also all elements containing the node in the mesh instance.
        ...
        for i in range(len(labels)):
            if label in conn[i]:
                elem2remove.append(labels[i])
        for l in elem2remove:
            self.drop_element(l)
    ...
    def add_set(self, label, elements):
        ...
        if type(label) is not str:
            raise Exception('set names must be strings.')
        label2 = label.lower()
        if label2 not in self.sets:
            self.sets[label2] = array(self.dti, [])
        ...
        for i in elements:
            if i in self.labels:
                ...
                print('Info: element {0} does not exist, it was not added to set {1}.'.format(
                    i, label2))
    ...
    def add_surface(self, label, description):
        ...
        if type(label) is not str:
            raise Exception(
                'Surface labels must be string, got {0} instead'.format(type(label)))
        if type(description) is not list:
            raise Exception(
                'Surface description must be list, got {0} instead'.format(type(description)))
        for i in range(len(description)):
            t = description[i]
            if len(t) != 2:
                raise Exception(
                    'Surface description elements must have 2 elements, got {0} instead'.format(len(t)))
            if type(t[0]) is not str:
                raise Exception(
                    '(In surface) Element sets keys must must be str, got {0} instead'.format(type(t[0])))
            if type(t[1]) not in [int]:
                raise Exception(
                    'Face numbers must keys must be int (generally in [1...6]), got {0} instead'.format(type(t[1])))
        if label not in self.surfaces.keys():
            self.surfaces[label] = []
    ...
    def dump2inp(self, path=None):
        ...
        for i in range(len(self.labels)):
            elements[self.name[i]].append(self.labels[i])
    ...
    def convert2tri3(self, mapping=None):
        ...
        for i in range(len(self.labels)):
            ...
            if s == 2:
                ...
                if len(c) == 4:
                    el2.add_element(
                        connectivity=[c[0], c[1], c[2]], space=s, name=n2, toset=sets)
                    el2.add_element(
                        connectivity=[c[2], c[3], c[0]], space=s, name=n2, toset=sets)
                if len(c) == 6:
                    el2.add_element(
                        connectivity=[c[0], c[3], c[5]], space=s, name=n2, toset=sets)
                    el2.add_element(
                        connectivity=[c[3], c[1], c[4]], space=s, name=n2, toset=sets)
                    el2.add_element(
                        connectivity=[c[3], c[4], c[5]], space=s, name=n2, toset=sets)
                    el2.add_element(
                        connectivity=[c[5], c[4], c[2]], space=s, name=n2, toset=sets)
                if len(c) == 8:
                    el2.add_element(
                        connectivity=[c[0], c[4], c[7]], space=s, name=n2, toset=sets)
                    el2.add_element(
                        connectivity=[c[4], c[1], c[5]], space=s, name=n2, toset=sets)
                    el2.add_element(
                        connectivity=[c[5], c[2], c[6]], space=s, name=n2, toset=sets)
                    el2.add_element(
                        connectivity=[c[7], c[6], c[3]], space=s, name=n2, toset=sets)
                    el2.add_element(
                        connectivity=[c[7], c[4], c[6]], space=s, name=n2, toset=sets)
                    el2.add_element(
                        connectivity=[c[4], c[5], c[6]], space=s, name=n2, toset=sets)
        return el2
    ...
    '''
        ...
        for ne in range(len(self.labels)):
            ...
            if self.space[ne] == 3:
                el = conn[ne]
                if len(el) == 4:
                    edges = [(el[0], el[1]), (el[1], el[2]), (el[2], el[0]),
                             (el[0], el[3]), (el[1], el[3]), (el[2], el[3])]
                if len(el) == 6:
                    edges = [(el[0], el[1]), (el[1], el[2]), (el[2], el[0]), 
                             (el[3], el[4]), (el[4], el[5]), (el[5], el[3]), 
                             (el[0], el[3]), (el[1], el[4]), (el[2], el[5])]
                if len(el) == 8:
                    edges = [(el[0], el[1]), (el[1], el[2]), (el[2], el[3]), 
                             (el[3], el[0]), (el[4], el[5]), (el[5], el[6]), 
                             (el[6], el[7]), (el[7], el[4]), (el[0], el[4]), 
                             (el[1], el[5]), (el[2], el[6]), (el[3], el[7])]
                for edge in edges:
                    revert = (edge[1], edge[0])
    ...
    def extrude(self, N=1, l=1., quad=False, mapping={}):
        ...
        for i in range(len(self.labels)):
            ...
            if space == 2:
                if quad == False:
                    if len(conn) in [3, 4]:
                        tempmesh.add_element(
                            label=label, connectivity=conn, space=space, name=name)
        for sk in self.sets.keys():
        ...
        if quad == False:
            for i in range(N):
                ...
                for ne in range(len(te.labels)):
                    ...
                    conn = [c + (i) * nmax for c in conn] + \
                        [c + (i + 1) * nmax for c in conn]
                    name = te.name[ne]
                    ...
                    omesh.add_element(
                        label=label, connectivity=conn, space=3, name=name)
                    for sk in tes.keys():
                        s = tes[sk]
                        if oldLabel in s:
    ...
    def sweep(self, N=1, sweep_angle=45., quad=False, mapping={}, extrude=False):
        ...
        # Delta to be applied along z axis
        delta = radians(float(sweep_angle) / N)
        inodes = self.nodes  # Input nodes
        omesh = Mesh(nodes=deepcopy(inodes))  # Output Mesh
        ...
        if quad == False:
            for layer in range(N):
                ...
                for ne in range(len(te.labels)):
                    ...
                    if nnoa == 0:
                        newconn = [c + (layer + 1) * nmax for c in oldconn] + \
                            [c + layer * nmax for c in oldconn]
                    if nnoa == 2:
                        if len(oldconn) == 3:
                            """
                            newconn = []
                            ...
                            while nodesOnAxis != [True, False, True]:
                                ...
                                nodesOnAxis = get_nodesOnAxis(oldconn)
                            newconn = [oldconn[0], oldconn[1] + layer * nmax,
                                       oldconn[2], oldconn[1] + (layer + 1) * nmax]

                        if len(oldconn) == 4:
                            while nodesOnAxis != [True, False, False, True]:
                                oldconn = [oldconn[-1], oldconn[0],
                                           oldconn[1], oldconn[2]]
                                nodesOnAxis = get_nodesOnAxis(oldconn)
                            newconn = [oldconn[0], layer * nmax + oldconn[1],
                                       (layer + 1) * nmax + oldconn[1], oldconn[3], layer * nmax + oldconn[2], (layer + 1) * nmax + oldconn[2]]
                    if newconn != None:
                        name = te.name[ne]
                        if name in mapping.keys():
                            name = mapping[name]
                        ...
                        omesh.add_element(
                            label=label, connectivity=newconn, space=3, name=name)
                    else:
                        print(
                            'element {0} has not been sweeped'.format(oldLabel))
        # Managing sets
        if quad == False:
            ins = self.nodes.sets
            onodes = omesh.nodes
            ...
            for i in range(len(inodes.labels)):
                ...
                if x != 0. or z != 0.:
                    frontnodes.append(oldLabel)
            omesh.nodes.add_set('front_nodes', frontnodes)
            omesh.nodes.add_set(
                'back_nodes', [nl + nmax * N for nl in frontnodes])
        # Managing surfaces:
        set_pattern = '{0}_f{1}_{2}'
        if quad == False:
            isurf = self.surfaces
    ...
    def dump2vtk(self, path=None):
        ...
        * Paraview plot: 
        ...
        nel = len(self.labels)
        for i in range(nel):
            esize += 1 + len(self.connectivity[i])
        out += 'CELLS {0} {1}\n'.format(nel, esize)
        eTypes = 'CELL_TYPES {0}\n'.format(nel)
        pattern = '{0} '
        pattern2 = '{0}\n'
        ...
        for key in fields.keys():
            if fields[key].position == "node":
                nfields[key] = fields[key]
            if fields[key].position == "element":
                efields[key] = fields[key]
        header = True
        ...
        for i in range(len(elabels)):
            ...
            if old in c:
                ...
                if new not in c:
                    ...
                    el = elabels[i]
                    print(
                        'Info: element {0} maybe have become degenerate due du node replacing.'.format(el))
  ...
  '''
    ...
    def union(self, other_mesh, crit_distance=None, simplify=True):
        ...
        if isinstance(other_mesh, Mesh) == False:
            raise Exception(
                'other_mesh should be Mesh instance, got {0}'.format(type(other_mesh)))
        ...
        if len(onodes.labels) != 0:
            for i in range(len(onodes.labels)):
                ...
                nodes.add_node(label=l + max_nlabel, x=x,
                               y=y, z=z, toset=toset)
            for i in range(len(other_mesh.labels)):
                ...
                name = element.name[0]
                self.add_element(
                    label=l + max_elabel, connectivity=conn, space=space, name=name, toset=toset)
            if simplify:
        ...
        for n in range(len(conn)):
            ...
            if s == 3:  # 3D
                ...
                if len(c) == 8:  # Hexahedron
                    ...
                    centroids[n] = (c0 * a0 + c1 * a1 + c2 * a2 + c3 * a3 +
                                    c4 * a4 + c5 * a5) / (a0 + a1 + a2 + a3 + a4 + a5)
        return centroids
    ...
    def draw(self, ax, field_func=None, disp_func=None, cmap=None, cmap_levels=20, cbar_label='Field', cbar_orientation='horizontal', edge_color="black", edge_width=1., node_style="k.", node_size=1., contour=False, contour_colors="black", alpha=1.):
        ...
        if field_func != None:
            ...
            grad = ax.tricontourf(X, Y, tri, field.data,
                                  cmap_levels, cmap=cmap, alpha=alpha)
            bar = plt.colorbar(grad, orientation=cbar_orientation)
            bar.set_label(cbar_label)
            if contour:
                ax.tricontour(X, Y, tri, field.data,
                              cmap_levels, colors=contour_colors)
    ...
    def node_set_to_surface(self, surface, nodeSet):
        ...
        for i in range(len(self.labels)):
            ...
            if self.space[i] == 2:
                if len(inter) >= 2:
                    if set([0, 1]) <= ind:
                        f1.append(self.labels[i])
                    if set([1, 2]) <= ind:
                        f2.append(self.labels[i])
                    if set([2, 3]) <= ind:
                        f3.append(self.labels[i])
                    if set([3, 0]) <= ind:
                        f4.append(self.labels[i])
        if len(f1) != 0:
...
def RegularQuadMesh(N1=1, N2=1, l1=1., l2=1., name='QUAD4', dtf='f', dti='I'):
    ...
    x, y = np.meshgrid(np.linspace(0, l1, int(N1 + 1)),
                       np.linspace(0, l2, int(N2 + 1)))
    ...
    for i in range(len(X)):
        nodes.add_node(None, X[i], Y[i], 0.)
    for i in range(len(C)):
        mesh.add_element(connectivity=(int(C[i]), int(
            C[i] + 1), int(C[i] + N1 + 2), int(C[i] + N1 + 1)), space=2, name=name)
    ...
    nodes.add_set('top', range(int((N1 + 1) * N2 + 1),
                  int((N1 + 1) * (N2 + 1) + 1)))
    nodes.add_set('left', range(1, int((N1 + 1) * N2 + 2), int(N1 + 1)))
    nodes.add_set('right', range(
        int(N1 + 1), int((N1 + 1) * (N2 + 1) + 1), int(N1 + 1)))
    return mesh
...
def RegularQuadMesh_like(x_list=[0., 1.], y_list=[0., 1.], name='QUAD4', dtf='f', dti='I'):
    ...
    for i in range(len(C)):
        mesh.add_element(connectivity=(
            C[i], C[i] + 1, C[i] + N1 + 2, C[i] + N1 + 1), space=2, name=name)
    nodes.add_set('bottomleft', 1)
