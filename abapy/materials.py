...
class VonMises(object):
    ...
    def __init__(self, labels='mat', E=1., nu=0.3, sy=0.01, dtf='d'):
        ...
        E = float_arg(E)
        if len(E) != l:
            raise Exception('Parameters must all have the same length')
        self.E = array(dtf, E)
        nu = float_arg(nu)
        if len(nu) != l:
            raise Exception('Parameters must all have the same length')
    ...
    def dump2inp(self):
        ...
        for i in range(len(self.E)):
            out += pattern.format(self.labels[i],
                                  self.E[i], self.nu[i], self.sy[i])
        return out[0:-1]
...
class Bilinear(object):
    ...
    def dump2inp(self):
        ...
        for i in range(len(self.E)):
            ...
            out += pattern.format(self.labels[i], self.E[i],
                                  self.nu[i], self.sy[i], self.Ssat[i], Eps_p_sat[i])
        return out[0:-1]
...
class SiDoLo(object):
    ...
    def dump2inp(self):
        ...
        for i in range(len(self.labels)):
            ...
            Lvarint = len(varint)
            out += pattern.format(self.labels[i],
                                  params['epsrk'], params['toly'], Lvarint)
        # La sortie est tout sauf le dernier element de out (ligne blanche)
        return out[0:-1]
