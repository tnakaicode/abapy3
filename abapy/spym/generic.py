...
class Spm_image(object):
  ...
  * max / min = {6:.2e} {9} / {7:.2e} {9}'''
    ...
    def plane_fit(self, mask=None):
        ...
        y = Y[coor]
        z = Z[coor]
        def fitfunc(p, x, y): return p[0] * x + \
            p[1] * y + p[2]  # Target function

        def errfunc(p, x, y, z): return fitfunc(p, x, y) - z  # Error function
        p0 = [0., 0., 0.]
    ...
    def line_fit(self, mask=False, direction='x'):
        ...
        for i in range(len(Z)):
            ...
            z_crop = z[coor]
            x_crop = x[coor]
            def fitfunc(p, x): return p[0] * x + p[1]  # Target function
            def errfunc(p, x, z): return fitfunc(p, x) - z  # Error function
            p0 = [0., 0.]
            p1, success = optimize.leastsq(
                errfunc, p0[:], args=(x_crop, z_crop))
            l = fitfunc(p1, x)
            z = z - l
            out.data[i] = z
            residual.data[i] = l
    ...
    def paraboloid_fit(self, mask=None):
        ...
        y = Y[coor]
        z = Z[coor]
        def fitfunc(p, x, y): return p[0] * x**2 + p[1] * \
            x + p[2] * y**2 + p[3] * y + p[4]  # Target function
    ...
    def crop(self, cx0=0, cx1=0, cy0=0, cy1=0):
        ...
        new_data[:] = np.nan
        cropped = Z[max(cy0, 0): min(ny0, ny0 - cy1),
                    max(cx0, 0): min(nx0, nx0 - cx1)]
        new_data[max(0, -cy0): max(0, -cy0) + len(cropped),
                 max(0, -cx0): max(0, -cx0) + len(cropped[0])] = cropped
        out = copy.deepcopy(self)
        out.data = new_data
    ...
    def is_like(self, other):
        ...
        if self.ly != other.ly:
            out = False
        if self.x_samples() != other.x_samples():
            out = False
        if self.y_samples() != other.y_samples():
            out = False
        if self.xc != other.xc:
            out = False
        if self.yc != other.yc:
...
Title   = {8}\n'''.format(
        ...
        data = ''
        for x in a:
            # Conversion to float32 in little endian binary
    ...
    for j in range(i):
        ...
        if field in known_fields:
            ...
            if field == 'XOffset':
                out.xc = - float(value)
            if field == 'XYUnits':
                out.xy_units = field.replace(
                    u'\u03bc', 'u')  # Changing mu to u
            if field == 'ZUnits':
                out.z_units = field.replace(u'\u03bc', 'u')  # Changing mu to u
